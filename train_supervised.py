...
from utils.utils import EarlyStopMonitor, MLP, NodeClassLayer
from myutils import get_neighbor_finder
...
parser.add_argument('--dec', type=str, default="old", choices=["old", "new"], help='decoder choice')
...
full_data, node_features, edge_features, edge_motif_features, train_data, val_data, test_data = \
  get_data_node_classification(DATA, THRESHOLD_TIME, use_validation=args.use_validation)

max_idx = max(full_data.unique_nodes)

_, train_ngh_finder = get_neighbor_finder(train_data, uniform=UNIFORM, max_node_idx=max_idx)
...
for i in range(args.n_runs):
  ...
  tme = TME(neighbor_finder=train_ngh_finder, node_features=node_features,
            edge_features=edge_features, edge_motif_features=edge_motif_features, device=device,
            n_layers=NUM_LAYER,
  ...
  logger.info('Loading saved TME model')
  model_path = f'./saved_models/{args.prefix}-{DATA}.pth'
  tme.load_state_dict(torch.load(model_path))
  tme.eval()
  logger.info('TME models loaded')
  logger.info('Start training node classification task')

  if args.dec == "old":
    decoder = MLP(node_features.shape[1], drop=DROP_OUT)
  else:
    decoder = NodeClassLayer(node_features.shape[1], 1, dropout=DROP_OUT)
  ...
  for epoch in range(args.n_epoch):
    ...
    for k in range(num_batch):
      ...
      with torch.no_grad():
        source_embedding, destination_embedding, _ = tme.compute_temporal_embeddings(sources_batch,
                                                                                     destinations_batch,
                                                                                     destinations_batch,
                                                                                     timestamps_batch,
                                                                                     edge_idxs_batch,
    ...
    val_auc = eval_node_classification(tme, decoder, val_data, full_data.edge_idxs, BATCH_SIZE,
  ...
  if args.use_validation:
    ...
    test_auc = eval_node_classification(tme, decoder, test_data, full_data.edge_idxs, BATCH_SIZE,
                                        n_neighbors=NUM_NEIGHBORS)
  else:
